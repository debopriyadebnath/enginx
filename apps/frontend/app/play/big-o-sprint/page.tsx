...
function BigOSprintInner() {
  ...
  const doSubmit = useCallback(async (forcedAnswer: string | null) => {
    ...
    const correct = answer !== null ? checkBigOAnswer(q, answer) : false;
    const pointsEarned = pointsForBigO(q, correct);
  ...
  const advance = useCallback(() => {
    if (index + 1 >= questions.length) {
      setPhase("done");
      toast.success(`Sprint done! +${runTotal} pts`);
      return;
    }
    setIndex((i) => i + 1);
  ...
  const timerColor = timeLeft > 6 ? "#6FFF00" : timeLeft > 3 ? "#f59e0b" : "#f43f5e";
    ...
    return (
      <div className="relative min-h-[100dvh] overflow-hidden">
        ...
        <div className="relative z-20 mx-auto max-w-2xl px-4 py-12">
          <div className="liquid-glass rounded-[28px] border border-white/12 p-8 text-center shadow-[0_32px_120px_rgba(0,0,0,0.65)] sm:p-10 [--glass-bg:rgba(0,0,0,0.75)] [--glass-bg-accent:rgba(111,255,0,0.05)]">
            <p className="font-mono text-xs uppercase tracking-widest text-cream/50">Big-O Sprint complete</p>
            <p className="font-grotesk mt-2 text-3xl uppercase text-neon sm:text-4xl">Results</p>
            <p className="mt-6 font-mono text-cream">Run score · <span className="text-xl font-semibold text-neon tabular-nums">{runTotal}</span> pts</p>
            ...
            {results.map((row, i) => (
              <li key={`${row.id}-${i}`} className={`liquid-glass rounded-[20px] border p-5 backdrop-blur-lg ${row.correct ? "border-emerald-500/30 [--glass-bg:rgba(0,0,0,0.65)] [--glass-bg-accent:rgba(16,185,129,0.08)]" : "border-rose-500/30 [--glass-bg:rgba(0,0,0,0.7)] [--glass-bg-accent:rgba(244,63,94,0.08)]"}`}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {row.pointsEarned > 0 && <span className="ml-auto font-mono text-xs text-neon">+{row.pointsEarned}</span>}
                </div>
                ...
                <p className="font-mono text-sm text-cream/90">Answer: <span className="text-neon">{row.correctAnswer}</span></p>
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            ...
            <Link href="/play/big-o-sprint" className="rounded-[12px] bg-neon px-5 py-3 font-grotesk uppercase text-[#010828] shadow-[0_0_24px_rgba(111,255,0,0.2)] hover:brightness-110">Play again</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden">
      <div className="fixed inset-0 z-0 bg-cover bg-center" style={{ backgroundImage: "url(/Game-bg.png)" }} />
      <div className="fixed inset-0 z-5 pointer-events-none opacity-[0.08]" style={{ backgroundImage: "url(/texture.png)", backgroundSize: "200px 200px" }} />
      <div className="pointer-events-none fixed inset-0 z-10 opacity-25" style={{ background: "radial-gradient(ellipse 60% 40% at 50% 0%, rgba(111,255,0,0.06), transparent 70%)" }} />
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-4 py-6 pb-16">
        ...
        <CodeTerminal title="complexity.c" subtitle="big-o sprint · identify the complexity">
          <pre className="font-mono text-sm text-cyan-100 leading-relaxed whitespace-pre-wrap">{q.code}</pre>
        </CodeTerminal>
        ...
        {feedback ? (
          <div className={`liquid-glass mt-6 rounded-[18px] px-5 py-4 ${feedback.correct ? "border-emerald-400/40" : "border-rose-400/40"}`}>
            ...
            <p className="font-mono text-sm text-cream/70 mt-1">Answer: <span className="text-neon">{q.answer}</span></p>
              ...
              {q.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border px-4 py-4 font-mono text-base transition backdrop-blur-md ${
                      active
                        ? "liquid-glass border-neon [--glass-bg:rgba(111,255,0,0.15)] text-neon shadow-[0_0_20px_rgba(111,255,0,0.2)]"
                        : "liquid-glass border-white/10 [--glass-bg:rgba(0,0,0,0.6)] text-cream hover:[--glass-bg:rgba(0,0,0,0.8)] hover:border-white/20"
                    }`}
                  >
                    {opt}
                  </button>
...
export default function BigOSprintPage() {
  return (
