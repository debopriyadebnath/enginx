...
function StructPadInner() {
  ...
  const doSubmit = useCallback(async (forcedAnswer: string | null) => {
    ...
    const correct = answer !== null ? checkStructAnswer(q, answer) : false;
    const pointsEarned = pointsForStruct(q, correct);
  ...
  const advance = useCallback(() => {
    if (index + 1 >= questions.length) {
      setPhase("done");
      toast.success(`Struct Pad done! +${runTotal} pts`);
      return;
    }
    setIndex((i) => i + 1);
  ...
  const timerColor = timeLeft > 10 ? "#38bdf8" : timeLeft > 5 ? "#f59e0b" : "#f43f5e";
    ...
    return (
      <div className="relative min-h-[100dvh] overflow-hidden">
        ...
        <div className="relative z-20 mx-auto max-w-2xl px-4 py-12">
          <div className="liquid-glass rounded-[28px] border border-white/12 p-8 text-center shadow-[0_32px_120px_rgba(0,0,0,0.65)] sm:p-10 [--glass-bg:rgba(0,0,0,0.75)] [--glass-bg-accent:rgba(56,189,248,0.05)]">
            <p className="font-mono text-xs uppercase tracking-widest text-cream/50">Struct Pad complete</p>
            <p className="font-grotesk mt-2 text-3xl uppercase text-sky-400 sm:text-4xl">Results</p>
            <p className="mt-6 font-mono text-cream">Run score · <span className="text-xl font-semibold text-sky-400 tabular-nums">{runTotal}</span> pts</p>
            ...
            {results.map((row, i) => (
              <li key={`${row.id}-${i}`} className={`liquid-glass rounded-[20px] border p-5 backdrop-blur-lg ${row.correct ? "border-emerald-500/30 [--glass-bg:rgba(0,0,0,0.65)] [--glass-bg-accent:rgba(16,185,129,0.08)]" : "border-rose-500/30 [--glass-bg:rgba(0,0,0,0.7)] [--glass-bg-accent:rgba(244,63,94,0.08)]"}`}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {row.pointsEarned > 0 && <span className="ml-auto font-mono text-xs text-sky-400">+{row.pointsEarned}</span>}
                </div>
                ...
                <p className="font-mono text-sm text-cream/90">Answer: <span className="text-sky-400">{row.correctAnswer}</span></p>
                {/* Memory layout visualization */}
                <div className="mt-3 rounded-[10px] bg-black/30 border border-white/10 px-3 py-2">
                  <p className="font-mono text-[10px] text-cream/40 mb-1 uppercase tracking-wider">Memory layout</p>
                  <pre className="font-mono text-xs text-amber-300/80 whitespace-pre-wrap">{row.layout}</pre>
                </div>
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            ...
            <Link href="/play/struct-pad" className="rounded-[12px] bg-sky-600 px-5 py-3 font-grotesk uppercase text-white shadow-[0_0_24px_rgba(56,189,248,0.2)] hover:bg-sky-500">Play again</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden">
      <div className="fixed inset-0 z-0 bg-cover bg-center" style={{ backgroundImage: "url(/Game-bg.png)" }} />
      <div className="fixed inset-0 z-5 pointer-events-none opacity-[0.08]" style={{ backgroundImage: "url(/texture.png)", backgroundSize: "200px 200px" }} />
      <div className="pointer-events-none fixed inset-0 z-10 opacity-20" style={{ background: "radial-gradient(ellipse 60% 40% at 50% 0%, rgba(56,189,248,0.08), transparent 70%)" }} />
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 liquid-glass px-4 py-3">
        <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="font-grotesk text-sm text-sky-400 uppercase tracking-wider">Struct Pad</span>
          </div>
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-4 py-6 pb-16">
        ...
        {/* Struct definition */}
        <CodeTerminal title="struct.c" subtitle="struct pad · calculate sizeof">
          <pre className="font-mono text-sm text-sky-100 leading-relaxed whitespace-pre-wrap">{q.code}</pre>
        </CodeTerminal>

        {/* Layout reveal toggle */}
        {!feedback && (
          <button
            type="button"
            onClick={() => setShowLayout((l) => !l)}
            className="mt-3 font-mono text-[11px] text-amber-400 hover:text-amber-300 transition-colors"
          >
            {showLayout ? "Hide memory layout ↑" : "Show memory layout hint ↓"}
          </button>
        )}
        {showLayout && !feedback && (
          <div className="mt-2 rounded-[10px] bg-black/30 border border-white/10 px-3 py-2">
            <p className="font-mono text-[10px] text-cream/40 mb-1 uppercase tracking-wider">Memory layout</p>
            <pre className="font-mono text-xs text-amber-300/80 whitespace-pre-wrap">{q.layout}</pre>
          </div>
        )}
        ...
        {feedback ? (
          <div className={`liquid-glass mt-6 rounded-[18px] px-5 py-4 ${feedback.correct ? "border-emerald-400/40" : "border-rose-400/40"}`}>
            ...
            <p className="font-mono text-sm text-cream/70 mt-1">Answer: <span className="text-sky-400">{q.answer}</span></p>
            ...
            <button
              type="button"
              onClick={advance}
              className="mt-4 w-full rounded-[12px] bg-sky-600 py-3 font-grotesk uppercase text-white shadow-[0_0_20px_rgba(56,189,248,0.3)] hover:bg-sky-500"
            >
          ...
          <>
              ...
              {q.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border px-4 py-4 font-mono text-sm transition text-center backdrop-blur-md ${
                      active
                        ? "liquid-glass border-sky-400 [--glass-bg:rgba(56,189,248,0.15)] text-sky-200 shadow-[0_0_20px_rgba(56,189,248,0.2)]"
                        : "liquid-glass border-white/10 [--glass-bg:rgba(0,0,0,0.6)] text-cream hover:[--glass-bg:rgba(0,0,0,0.8)] hover:border-sky-500/40"
                    }`}
                  >
                    {opt}
                  </button>
            ...
            <button
              ...
              className="mt-8 w-full rounded-[14px] bg-sky-600 py-4 font-grotesk uppercase tracking-wide text-white shadow-[0_0_28px_rgba(56,189,248,0.3)] hover:bg-sky-500 disabled:opacity-40"
            >
...
export default function StructPadPage() {
  return (
